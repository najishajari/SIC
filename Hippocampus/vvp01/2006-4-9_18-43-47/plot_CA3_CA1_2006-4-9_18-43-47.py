...
for i in range(32):
    for j in range(32):
        f=open(cluster_path+'/vvp01/2006-4-9_18-43-47/output/'+str(i)+','+str(j)+'.txt')
        f_GC=open(cluster_path+'/vvp01/2006-4-9_18-43-47/GC_output/'+str(i)+','+str(j)+'.txt')
        for k in range(300):
            line= f.readline()
            ...
            res_GC[i,j,k]= np.float64(line_GC)[1]>np.float64(line_GC)[0]
            #res_2[i,j,k]= np.float64(line_GC)[0]<np.float64(line_GC)[1]
...
e=plt.errorbar(np.arange(0,trials,1),res_mean,yerr=[below_err,top_err],color='b',ecolor='#990000',fmt='-o',label=r'${\rm SIC}$')
...
e_GC=plt.errorbar(np.arange(0,trials,1),res_GC_mean,yerr=[below_err_GC,top_err_GC],color='r',ecolor='#990000',fmt='-o',label=r'${\rm WG}$')
plt.xlim(-1,300)
...
plt.savefig(manuscript_path+'Linear_Filters/Figures/SIC-WG-vvp01_2006-4-9_18-43-47.eps',transparent=True,dpi=500)
...
#plt.figure()
#p=plt.imshow(res_2[:,:,36],extent=[0,32,0,32],origin='lower',alpha=1,aspect='1',interpolation='none')
